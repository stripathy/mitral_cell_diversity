function [tsp,Vmem,Ispk, neglogli] = simGLM(glmprs,Stim);
...
if size(glmprs.k,3) > 1  % Run "coupled" GLM model if multiple cells present
    ...
    if nargout <= 2
        ...
        [tsp,Vmem,Ispk, neglogli] = simGLMsingle(glmprs,Stim);
    end
end
    
% ======================================================
function [tsp,Vmem,Ispk, neglogli] = simGLMsingle(glmprs,Stim);
...
% --------------- Check Inputs ----------------------------------
% global RefreshRate;
RefreshRate = glmprs.RefreshRate;
...
while jbin <= rlen
    ...
    if (tspnext >= rrcum(end)) % No spike in this window
        ...
        if ~isempty(iiPostSpk)
            ...
            if nargout > 2  % Record post-spike current
...
% compute the likeli of the spiketrain just computed given the model and
% stim
if nargout > 3
    rr = nlfun(Vmem);
    SPNDS = round(tsp/dt);
    trm1 = -sum(log(rr(SPNDS)));  % Spiking term
    trm2 = sum(rr)*dt/RefreshRate;  % non-spiking term
    neglogli = trm1 + trm2;
end

% rewrite of bayesStimDecoder

function [optStim, exitflag, fval, hessian] = bayesStimDecoder2(cellModels, spikeTimes, stimInterval, dtDecoding, stimCovMat, initStim)

numCells = length(cellModels);
% tPts = dtDecoding:dtDecoding:length(initStim)*dtDecoding;
tPts = dtDecoding:dtDecoding:length(initStim);
...
%     kMat(:,i) = fastinterp2(cellModels(i).k,round(1/dtDecoding)) * dtDecoding; %this will need to get rescaled to match dtDecoding
    kMat(:,i) = decimate(cellModels(i).k, dtDecoding) * dtDecoding; %the extra dtDecoding mult seems justified
    hTermKernel = cellModels(i).ihbas*cellModels(i).ih;
%     hTermKernel = cellModels(i).ih;
    hTermKernel = decimate(hTermKernel,round(dtDecoding/cellModels(i).dt)) ;
    hTermKernel(1) = 0;
...
if nargin > 5
    currEstStim = downsample(initStim, dtDecoding);
else

% make figures for greedySearch stuff
...
for i = 2:numSearchIters
    [stimPval(i), signifValsStim(i)] = ranksum(stimDiffAll(:,i), stimDiffRand(:,popSize == i), 'alpha', .01);
    [histPval(i), signifValsHist(i)] = ranksum(histDiffAll(:,i),histDiffRand(:,popSize == i), 'alpha', .01);
    [biasPval(i), signifValsBias(i)] = ranksum(biasDiffAll(:,i), biasDiffRand(:,popSize == i), 'alpha', .01);
end
    
signifValsStim = find(signifValsStim==1);
signifValsHist = find(signifValsHist==1);
signifValsBias = find(signifValsBias==1);
...
for i = 1:3
    ...
    if i == 1;
        ...
        bestSignifVals = signifValsStim;
        axisVals = [1 numSearchIters+1 -.2 1.35];
        plotSignifVal = 1.3;
    elseif i == 2;
...
%     boundedline(validInds, bestMean, bestStdErr, 'b', 'transparency', .2);
    ...
    plot(bestSignifVals, plotSignifVal, 'b*');
    axis(axisVals);
